...
layout(location = 0) in vec3 te_position_world;
layout(location = 1) in vec3 te_normal_world;
...
float calculateDitheredShadow(vec4 lightSpacePos, vec3 normal, vec3 lightDir) {
    ...
    // clip everything around shadowMap
    ...
    if (slope < 0.5) {
        ...
        const float biasFlat  = 0.0002;  
        const float biasSlope = 0.02;   
        bias = biasFlat + biasSlope * slope;
        bias = clamp(bias, biasFlat, biasFlat + biasSlope); 
...
void main() {
    ...
    vec3 viewDir = normalize(camera_world - te_position_world);
    ...
    // Point light
    vec3 pointVec = pointL.position - te_position_world;
    ...
    // Fog
    float distance = length(camera_world - te_position_world); 
    float fogStart = 0.0;
    ...
    float planeFogAmount = getPlaneFogAmount(te_position_world);
    ...
    FragColor = vec4(result, 1.0 - combinedFog * 0.5); // subtracting fog from alpha value to blend into bg
